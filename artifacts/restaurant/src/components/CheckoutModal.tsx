...
import { X, User, Phone, MapPin, ShoppingBag, CheckCircle, Copy, Tag } from "lucide-react";
import { useOrders } from "@/hooks/useOrders";
import { useCart } from "@/contexts/CartContext";
import { useLoyalty } from "@/hooks/useLoyalty";
import { useGiftCards } from "@/hooks/useGiftCards";
import { isFirebaseConfigured } from "@/lib/firebase";
import { OrderType, GiftCard } from "@/types";
import { RedeemGiftCard } from "./GiftCardWidget";
...
export default function CheckoutModal({ isOpen, onClose, onSuccess, tipAmount = 0 }: Props) {
  const { items, total, clearCart, addToHistory, giftCard, setGiftCard } = useCart();
  ...
  const [completedOrder, setCompletedOrder] = useState<Parameters<typeof AnimatedReceipt>[0]["order"]>(null);

  const deliveryFee = form.orderType === "delivery" ? 3.99 : 0;
  const TAX_RATE = 0.0875;
  const tax = total * TAX_RATE;
  const giftDiscount = giftCard ? Math.min(giftCard.discount, total + deliveryFee + tax + tipAmount) : 0;
  const grandTotal = Math.max(0, total + deliveryFee + tax + tipAmount - giftDiscount);

  const set = (key: keyof typeof form, value: string) =>
    setForm((f) => ({ ...f, [key]: value }));

  const handleGiftCard = (card: GiftCard) => {
    setGiftCard({ id: card.id, code: card.code, discount: Math.min(card.balance, total + deliveryFee + tax + tipAmount) });
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const orderItems = items.map(({ meal, quantity }) => ({
        mealId: meal.id, mealName: meal.name, mealImage: meal.imageUrl, price: meal.price, quantity,
      }));
      const orderNumber = await placeOrder({
        ...
        giftCardDiscount: giftDiscount || undefined,
        total: grandTotal,
      });

      if (giftCard && giftDiscount > 0) {
        await redeemGiftCard(giftCard.id, giftDiscount).catch(() => {});
      }
      await addStamp(form.customerPhone.trim(), form.customerName.trim(), grandTotal).catch(() => {});
  ...
  return (
    <>
                ...
                <div className="p-5 space-y-5">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input className={inputCls} style={{ ...inputStyle, paddingLeft: "2.25rem" }}
                        placeholder="e.g. Alex Johnson" value={form.customerName}
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input className={inputCls} style={{ ...inputStyle, paddingLeft: "2.25rem" }}
                        placeholder="+1 555 000 0000" type="tel" value={form.customerPhone}
                  ...
                  {form.orderType === "delivery" && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}>
                      ...
                      <div className="relative">
                        ...
                        <textarea className={inputCls} style={{ ...inputStyle, paddingLeft: "2.25rem", resize: "none" }}
                          placeholder="Street, City, Zip..." rows={2} value={form.deliveryAddress}
                  ...
                  <div>
                    ...
                    <textarea className={inputCls} style={{ ...inputStyle, resize: "none" }}
                      placeholder="Allergies, extra requests..." rows={2} value={form.notes}
                  ...
                  <div>
                    ...
                    <RedeemGiftCard
                      onValidated={handleGiftCard}
                      onRemove={() => setGiftCard(null)}
                      appliedCard={giftCard}
                      orderTotal={total + deliveryFee + tax + tipAmount}
                    />
                  </div>
                  ...
                  <div className="rounded-2xl p-4 space-y-2"
                    ...
                    <div className="pt-2 mt-2 space-y-1.5" style={{ borderTop: "1px solid rgba(255,255,255,0.07)" }}>
                      ...
                      {giftDiscount > 0 && (
                        <div className="flex justify-between text-sm">
                          <span style={{ color: "#10b981" }}>Gift Card</span>
                          <span style={{ color: "#10b981" }}>-${giftDiscount.toFixed(2)}</span>
                        </div>
                      )}
      ...
      <AnimatedReceipt order={completedOrder} onClose={() => setCompletedOrder(null)} />
    </>
  );
}
  ...
  return (
    ...
    >
      <motion.div
        ...
        style={{ background: "linear-gradient(180deg, #0c1a2e 0%, #10082a 100%)", border: "1px solid rgba(255,255,255,0.1)" }}
      >
        <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.15, type: "spring", damping: 16 }}
        ...
        <p className="text-white/50 text-sm mb-6">Your food is being prepared. Track your order below.</p>

        <div className="rounded-2xl p-4 mb-6"
          style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)" }}>
          <p className="text-white/40 text-xs uppercase tracking-wider mb-2">Your Order Number</p>
          <div className="flex items-center justify-center gap-3">
            <span className="text-3xl font-extrabold tracking-widest" style={{ color: "#a78bfa", fontFamily: "monospace" }}>
              {orderNumber}
            </span>
            <button onClick={copy} className="p-2 rounded-xl" style={{ background: "rgba(255,255,255,0.06)" }}>
              {copied ? <CheckCircle size={16} style={{ color: "#10b981" }} /> : <Copy size={16} className="text-white/50" />}
            </button>
          </div>
        ...
        <div className="flex gap-3">
          <a href="/track" className="flex-1 py-3 rounded-2xl text-sm font-semibold text-white text-center"
